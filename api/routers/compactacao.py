"""
Precision VRT Solo — Router de Compactação

Endpoints HTTP para compactação.
...
from app.services import compactacao_service

router = APIRouter()

# Instância do service
compactacao_service = compactacao_service.CompactacaoService()
...
def importar_arquivos(
    ...
    """
    Importa arquivos para compactação.
    """
    try:
        ...
        # Chamar service
        resultado = compactacao_service.importar_arquivos(
...
def interpolar_dados(
    ...
    """
    Processa interpolação de dados.
    """
    try:
        # Validação de parâmetros
        validar_parametros_obrigatorios({
        ...
        # Chamar service
        resultado = compactacao_service.processar_interpolacao(
            arquivo_limite.filename,
            arquivo_amostras.filename,
            configuracoes
        )
...
def zonear_area(
    ...
    try:
        ...
        # Chamar service
        resultado = compactacao_service.processar_zoneamento(
            arquivo_limite.filename,
            configuracoes
        )
...
def gerar_mapa(
    ...
    try:
        ...
        # Chamar service
        resultado = compactacao_service.gerar_mapa(
            arquivo_limite.filename,
...
def exportar_resultados(
    ...
    try:
        # Validação de parâmetros
        validar_parametros_obrigatorios({
            "dados_compactacao": "dados",
            "formatos": formatos,
            "nome_arquivo_base": nome_arquivo_base
        }, ["dados_compactacao", "formatos", "nome_arquivo_base"])
        
        # Chamar service
        resultado = compactacao_service.exportar_resultados(
            dados_compactacao,
            formatos,
            nome_arquivo_base,
            configuracoes
...
def pipeline_completo(
    ...
    """
    Executa pipeline completo de compactação.
    """
    try:
        # Validação de parâmetros
        validar_parametros_obrigatorios({
        ...
        resultado = compactacao_service.processar_compactacao(
            arquivo_limite.filename,
            arquivo_amostras.filename,
            configuracoes
        )
