...
class Interpolador:
    ...
    def interpolar_talhao(
        ...
        **kwargs: Any,
        ...
        grid_x, grid_y = self._gerar_malha(x, y)
    ...
    def _gerar_malha(
        ...
        """Gera a malha de interpolação."""
        xmin, xmax = float(x.min()), float(x.max())
        ymin, ymax = float(y.min()), float(y.max())

        # Resolução aproximada em graus (~10m = 10/111320 graus)
        resolucao_graus = self.resolucao_m / 111320.0
        ...
        logger.info("Malha gerada: %d x %d (%d pixels).", nx, ny, nx * ny)
    ...
    def _interpolar_atributo(
        ...
        if metodo == "rbf":
            pred = self._interpolar_rbf(xy_norm, z_valid, grid_norm, grid_x.shape)
        elif metodo == "idw":
            pred = self._interpolar_idw(xy_norm, z_valid, grid_norm, grid_x.shape)
        elif metodo == "nearest":
